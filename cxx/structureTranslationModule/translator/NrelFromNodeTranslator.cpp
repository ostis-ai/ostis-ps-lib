...
std::stringstream NrelFromNodeTranslator::translate(ScAddr const & structAddr) const
{
  ...
  context->HelperSmartSearchTemplate(
      ...
      [&](ScTemplateResultItem const & searchResult)
      {
        ...
        if (context->HelperCheckEdge(TranslationKeynodes::translation_ignored_keynodes, nrelNode, ScType::EdgeAccessConstPosPerm))
  ...
  SC_LOG_INFO("NrelFromNodeTranslator: " << translations.str());
  return translations;
}

std::string NrelFromNodeTranslator::get_translation_of_relation(ScAddr const & structAddr, ScAddr const & nrelNode, ScAddr const & sourceNode) const
{
  ScAddr node;
